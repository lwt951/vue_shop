<template>
  <div>
    ...
    <!-- 添加用戶彈出框 -->
          ...
          <el-dialog title="添加用戶" :visible.sync="addDialogVisible" @close="$refs.addFormRef.resetFields()">
              <!-- 彈出框主體 -->
     ...
     <!-- 修改用戶彈出框 -->
      <el-dialog title="修改用戶" :visible.sync="editDialogVisible" @close="$refs.editFormRef.resetFields()">
              <!-- 彈出框主體 -->
...
export default {
  data() {
     ...
     var checkMail = (rule, value, callback) => {
         ...
         if (!emailRules.test(value)) return callback(new Error('電子郵件格式錯誤'))
         callback()
     }
     // 驗證手機的規則
    ...
    // 添加用戶的表單數據
      ...
      addFormRules: {
        ...
        username: [
          ...
          { min: 3, max: 10, message: '用戶名稱長度為3~10字符之間', trigger: 'blur' }
        ],
        // 密碼驗證
        password: [
          { required: true, message: '請輸入密碼', trigger: 'blur' },
          { min: 6, max: 15, message: '密碼長度為6~15字符之間', trigger: 'blur' }
        ],
        email: [
  ...
  methods: {
    ...
    async removeUser(userInfo) {
      const confirmResult = await this.$confirm(`確認刪除 ${userInfo.username} ?`, '提示', {
